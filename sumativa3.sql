...
SELECT 
    ...
    CONCAT('Producto: ', nombre) AS nombre_producto,
    precio
FROM 
    Productos
ORDER BY 
...
SELECT
    ROUND(AVG(CANTIDAD)) AS promedio_cantidad 
FROM VENTAS;
...
SELECT 
    ...
    ROUND(AVG(cantidad)) AS promedio_ventas_mensuales,
    SUM(cantidad) AS total_ventas_mes
FROM 
    Ventas
GROUP BY 
    EXTRACT(YEAR FROM fecha), EXTRACT(MONTH FROM fecha)
ORDER BY 
    total_ventas_mes DESC
FETCH FIRST 1 ROWS ONLY; -- Cambiado para usar ROWNUM


SELECT * 
FROM clientes
INNER JOIN ventas
ON ventas.cliente_id = 10;
